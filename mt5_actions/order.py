...
def buy_order(prediction,symbol):
    ...
    print("1. order_send(): by {} {} lots at {} with deviation={} points".format(symbol,lot,mt5.symbol_info_tick(symbol).ask,deviation));
...
def sell_order(prediction,symbol):
    ...
    result = mt5.order_send(request)
    # check the execution result
    print("1. order_send(): by {} {} lots at {} with deviation={} points".format(symbol,lot,mt5.symbol_info_tick(symbol).bid,deviation));
    if (result.retcode not in  [mt5.TRADE_RETCODE_DONE, mt5.TRADE_RETCODE_PLACED]) :
